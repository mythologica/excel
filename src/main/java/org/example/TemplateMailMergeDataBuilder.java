package org.example;

import org.example.common.mailreqex.MailMergeDataBuilder;
import org.example.common.mailreqex.vo.MailMergeConfig;
import org.example.common.mailreqex.vo.MailMergeData;
import org.example.common.mailreqex.vo.MailMergeFindKey;

import java.util.LinkedHashMap;
import java.util.List;
import java.util.Map;

public class TemplateMailMergeDataBuilder implements MailMergeDataBuilder  {

    private Map<String,String> defaultData = new LinkedHashMap<>();

    public TemplateMailMergeDataBuilder(Map<String,String> defaultData) {
        this.defaultData = defaultData;
        //TODO: 데이터 베이스 조회하는 로직 넣을 것
    }

    @Override
    public MailMergeData createMergeData(MailMergeConfig mailMergeConfig) {
        MailMergeData mailMergeData = new MailMergeData();

        if (mailMergeConfig.getMailMergeFindKeys().size() > 0) {
            if (mailMergeConfig.getMailMergeFindKeysByUser() != null) {
                initParsingDatasByUser(mailMergeData);
            }

            if (mailMergeConfig.getMailMergeFindKeysyQrCode() != null) {
                initParsingDatasByQrCode(mailMergeData);
            }

            if (mailMergeConfig.getMailMergeFindKeysOptOut() != null) {
                initParsingDatasByMarkingOptOut(mailMergeData);
            }

            if (mailMergeConfig.getEtcMailMergeFindKeys() != null && mailMergeConfig.getEtcMailMergeFindKeys().size() > 0 ) {
                for(MailMergeFindKey mailMergeFindKey: mailMergeConfig.getEtcMailMergeFindKeys()) {
                    initParsingDatasByEtc(mailMergeData,mailMergeFindKey);
                }
            }
        }

        System.out.println("mailMergeData:"+mailMergeData);

        return mailMergeData;
    }

    private MailMergeData initParsingDatasByUser(MailMergeData mailMergeData) {

        //TODO: database 조회 넣을 것

        mailMergeData.put("user", "country", "kr");
        mailMergeData.put("user", "id", "hglqhdytYIDFafdhjf");
        mailMergeData.put("user", "name", "홍길동");
        mailMergeData.put("user", "email", "test@mail.com");

        return mailMergeData;
    }

    private MailMergeData initParsingDatasByQrCode(MailMergeData mailMergeData) {
        //TODO: database 조회 넣을 것

        mailMergeData.put("qrcode", "src", "https://www.google.com/images/branding/googlelogo/2x/googlelogo_color_272x92dp.png");
        mailMergeData.put("qrcode", "srcBase64", "data:image/png;base64,");
        mailMergeData.put("qrcode", "alt", "Google");
        mailMergeData.put("qrcode", "fromDate", "2023-05-12");
        mailMergeData.put("qrcode", "toDate", "2023-05-31");

        return mailMergeData;
    }

    private MailMergeData initParsingDatasByMarkingOptOut(MailMergeData mailMergeData) {

        mailMergeData.put("markOptOut", "link", "https://localhost:9090/markingOptOut/##user.id##");
        mailMergeData.put("markOptOut", "label", "link");


        return mailMergeData.merge((data) -> {
            String userId = data.getString("user", "id");

            //값에서 치환해야 할 것 정의
            String link = data.getString("markOptOut", "link").replaceAll("##user.id##", userId);
            data.put("markOptOut", "link", link);

            return data;
        });
    }

    private MailMergeData initParsingDatasByEtc(MailMergeData mailMergeData , MailMergeFindKey mailMergeFindKey ) {
        //TODO: 키값에 DS_EVENT_MAIL_MERGEDATA

        return mailMergeData;
        /*
        mailMergeData.put("markOptOut", "link", "https://localhost:9090/markingOptOut/##user.id##");
        mailMergeData.put("markOptOut", "label", "link");

        return mailMergeData.merge((data) -> {
            String userId = data.getString("user", "id");

            //값에서 치환해야 할 것 정의
            String link = data.getString("markOptOut", "link").replaceAll("##user.id##", userId);
            data.put("markOptOut", "link", link);

            return data;
        });
        */
    }
}
